...
### validate( ogrnStr )
Валидирует строку `ogrnStr` на соответствие исходным данным `config`. Возвращает валидационный объект `validate`, поля которого получают boolean значения.

#### validate ( валидационный объект )
...
###### initVal
Получает значение `true`, если 1-й знак валидируемой строки `ogrnStr` совпадает с одним из элеметов `config.initVals`. [Подробнее о config.initVals](#initvals). В остальных случаях получает значение `false`.
